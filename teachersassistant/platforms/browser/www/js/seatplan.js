          ...
          ref.orderByChild('subjectid').equalTo(subjectid)
           .once('value').then(function(snapshot) {
               snapshot.forEach(function(childSnapshot) {
                ...
                if (childData.subjectid == subjectid && childData.fullname == student_fullname && childData.date != today) {
                ...
                else if(childData.subjectid == subjectid && childData.fullname == student_fullname && childData.date == today) 
                {  
...
$$(document).on('pageInit',function(e){
    ...
    if (page.name === 'seatplan') {
        ...
        $$('#date_today').text(date);
         ...
         var ref = firebase.database().ref("seatplan"); //root reference to your data
          ref.orderByChild('student_fullname')
           .once('value').then(function(snapshot) {
               snapshot.forEach(function(childSnapshot) {
                var key = childSnapshot.key;
                var childData = childSnapshot.val();
