...
function viewGrades(studentname, subjectid) {
  ...
  ref.orderByChild('subjectid').equalTo(subjectid)
     .once('value').then(function(snapshot) {
          content1 += '<table class="gradestable">';
          content1 += '<tr><th style="text-align:center;">Date Attended</th> <th> Presence </th></tr>';
          content1 += ' <tbody>';
           ...
           snapshot.forEach(function(childSnapshot) {
            ...
            if (childData.subjectid == subjectid && childData.fullname == studentname) {
              if (!content1.includes(childData.date)) {
                 content1 += '<tr><td style="text-align:center;">'+ childData.date + '</td> <td>'+ childData.status + '</td></tr>';
              }
  ...
  ref.orderByChild('subjectid').equalTo(subjectid)
   .once('value').then(function(snapshot) {
      ...
      content += '<table class="gradestable">';
      content += '<thead';
      content += ' <tbody>';
